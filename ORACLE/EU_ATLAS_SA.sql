WITH OORDDTA AS
  ...
  AND TO_NUMBER(TO_CHAR (CAST(from_tz(CAST(ORD_DT AS TIMESTAMP),'America/Detroit') at TIME zone 'Africa/Johannesburg' AS TIMESTAMP),'YYYYMM'))=202003
  ),
  -----OORDLIN
  OORDLIN AS
  (SELECT ORD_NO,
    ...
    CAST (EXTEND_IBO_PRICE AS NUMBER)                      AS ADJ_LN_LC_NET,
...
SELECT OORDDTA.OPER_AFF_ID,
  ...
  CAST ('true' AS VARCHAR2(5)) PAY_REQ_FLAG, -- недоступно --поле отсутствует в Атомик (есть в самом Atlas по словам СА менеджеров)
