SELECT *
      ...
      SELECT OORDDTA.OPER_AFF_ID,
        ...
        CASE
          ...
          THEN NVL(OORDLIN.ORD_ACCOUNT_ID, OORDDTA.ORD_ACCOUNT_ID)
          WHEN NVL(OORDLIN.ORD_ACCOUNT_ID, OORDDTA.ORD_ACCOUNT_ID)       <> NVL(OORDLIN.VOL_ACCOUNT_ID, OORDDTA.VOL_ACCOUNT_ID)
          AND MRMS_INVOICES.ORD_BUS_NATR_CD <> 3
          THEN NVL(OORDLIN.VOL_ACCOUNT_ID, OORDDTA.VOL_ACCOUNT_ID)
          WHEN NVL(OORDLIN.ORD_ACCOUNT_ID, OORDDTA.ORD_ACCOUNT_ID)                    = OORDLIN.VOL_ACCOUNT_ID
          AND COALESCE(MRMS_INVOICES.ORD_BUS_NATR_CD, 1) = 1
          THEN NVL(OORDLIN.VOL_ACCOUNT_ID, OORDDTA.VOL_ACCOUNT_ID)
          ELSE NVL(OORDLIN.VOL_ACCOUNT_ID, OORDDTA.VOL_ACCOUNT_ID)
        END AS ACCOUNT_ID,
        CAST (
        CASE
        ...
        NVL(OORDLIN.VOL_ACCOUNT_ID, OORDDTA.VOL_ACCOUNT_ID) AS VOL_ACCOUNT_ID,
        NVL(OORDLIN.ORD_ACCOUNT_ID, OORDDTA.ORD_ACCOUNT_ID) AS ORD_ACCOUNT_ID,
        NVL(OORDLIN.SHP_ACCOUNT_ID, OORDDTA.SHP_ACCOUNT_ID) AS SHP_ACCOUNT_ID,
      ...
      AND NVL(OORDLIN.VOL_ACCOUNT_ID, OORDDTA.VOL_ACCOUNT_ID)=MRMS_INVOICES.VOL_ACCOUNT_ID
        )
