/*Load from DESEAI01.DEMAND_SALES_DET_RAW. ANZ not included*/
DECLARE YM NUMBER:=0;
BEGIN
 FOR i IN 24092..24252
...
FROM
  ...
  AND OPER_CNTRY_KEY_NO IN (23,134,56,39,76,34,74,75,17,54,48,16,44,57,10,8,47,18,14,11,46,45,30,37,60,59,52,29,27,13,32,6,114,41)
  ) FACT_T
...
LEFT JOIN
  ...
  WHERE CNTRY_KEY_NO IN (23,134,56,39,76,34,74,75,17,54,48,16,44,57,10,8,47,18,14,11,46,45,30,37,60,59,52,29,27,13,32,6,114,41)
  ) ORD_DIM
...
SELECT CNTRY_DIM.AFF_ID                          AS OPER_AFF_ID,
  ...
  GDW.ORD_WHSE_CD,
  GDW.SHP_POSTAL_CD,
  MAST_T_BILL.IMC_KEY_NO AS ACCOUNT_ID,
  CAST(NULL AS VARCHAR2(8 BYTE)) AS ACCOUNT_TYPE_ORD,
  MAST_T_VOL.IMC_KEY_NO  AS VOL_ACCOUNT_ID,
  MAST_T_ORD.IMC_KEY_NO  AS ORD_ACCOUNT_ID,
  MAST_T_SHP.IMC_KEY_NO  AS SHP_ACCOUNT_ID,
  GDW.ORD_CANC_FLAG,
  GDW.ORD_SHP_FLAG,
  GDW.PAY_REQ_FLAG,
...
LEFT JOIN
  ...
  WHERE IMC_CNTRY_KEY_NO IN (23,134,56,39,76,34,74,75,17,54,48,16,44,57,10,8,47,18,14,11,46,45,30,37,60,59,52,29,27,13,32,6,114,41)
  ) MAST_T_VOL
