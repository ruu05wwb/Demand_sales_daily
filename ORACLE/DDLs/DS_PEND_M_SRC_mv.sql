CREATE MATERIALIZED VIEW DS_PEND_M_SRC
 ...
 NOCOMPRESS LOGGING
  ...
  AS
SELECT 
  COALESCE(EU_AS400, 0) AS EU_AS400,
...
FROM
  (SELECT SRC,
    ORD_MO_YR_ID,
    MAX(UPDT_ON_LAST_RUN_FLG) OVER(PARTITION BY ORD_MO_YR_ID) AS UPDT_ON_LAST_RUN_FLG,
    DENSE_RANK() OVER(ORDER BY ORD_MO_YR_ID DESC) AS MNTH_RNK
  FROM DWSEAI01.DS_PENDING_MONTHS WHERE UPDT_ON_LAST_RUN_FLG=0
