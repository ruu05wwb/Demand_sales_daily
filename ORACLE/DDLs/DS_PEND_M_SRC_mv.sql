...
FROM
  ...
  FROM
    (SELECT SRC,
      ORD_MO_YR_ID,
      CASE WHEN DENSE_RANK() OVER(ORDER BY ORD_MO_YR_ID DESC)<=2 THEN 0 ELSE UPDT_ON_LAST_RUN_FLG END AS UPDT_ON_LAST_RUN_FLG,
      DENSE_RANK() OVER(ORDER BY ORD_MO_YR_ID DESC) AS MNTH_RNK
    FROM DWSEAI01.DS_PENDING_MONTHS ORDER BY SRC, ORD_MO_YR_ID
    )
